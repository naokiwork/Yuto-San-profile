...
import TabNav from '@/components/TabNav'; // Re-add TabNav import
...
export default function Home() {
  ...
  const academicIDs = [
    { icon: <FaGraduationCap className="text-muted" />, href: "#", label: "Google Scholar" },
    { icon: <FaUniversity className="text-muted" />, href: "#", label: "ResearchGate" },
    { icon: <FaGraduationCap className="text-muted" />, href: "#", label: "ORCID" },
  ];
    ...
    <>
      ...
      <Module id="overview" className="md:min-h-[calc(100vh-theme(spacing.20))] flex items-center bg-bg-base relative overflow-hidden">
        ...
        <Container className="grid grid-cols-1 gap-8 items-center text-center relative z-10">
          ...
          <Reveal delay={100}>
            <p className="text-h3 text-muted mb-4">{profile.title} at {profile.affiliation}</p>
          </Reveal>
          <Reveal delay={200}>
            <p className="text-h3 text-text max-w-prose mx-auto mb-8">{profile.bio}</p>
          </Reveal>
          <Reveal delay={300}>
      ...
      {/* Publications */}
      <Module id="publications" className="bg-surface-2">
        <Container className="max-w-md-container">
          <Reveal>
            <SectionHeading title="Publications" subtitle="Recent academic contributions and papers." className="text-text" />
          </Reveal>
          <PublicationList publications={publicationsData} className="bg-surface-2" />
        </Container>
      </Module>
      ...
      <Module id="projects" className="bg-bg-base">
        <Container className="max-w-md-container">
          <Reveal>
            <SectionHeading title="Projects Gallery" subtitle="Showcasing my work with real-world applications." className="text-text" />
          </Reveal>
      ...
      <Module id="code" className="bg-surface-2">
        <Container className="max-w-md-container">
          <Reveal>
